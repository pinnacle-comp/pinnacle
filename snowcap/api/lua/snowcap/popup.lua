...
---@field layer? snowcap.layer.LayerHandle Popup's parent surface is a Layer.
---@field popup? snowcap.popup.PopupHandle Popup's parent surface is another Popup.
local ParentHandle = {}
...
function popup.new_widget(args)
    ...
    if args.parent.layer then
        request.layer_id = args.parent.layer.id
    elseif args.parent.popup then
        request.popup_id = args.parent.popup.id
    else
...
function PopupHandle:on_key_pressed(on_press)
    local err = client:snowcap_input_v1_InputService_KeyboardKey(
        { popup_id = self.id },
        function(response)
            ---@cast response snowcap.input.v1.KeyboardKeyResponse
