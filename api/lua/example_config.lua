...
require("pinnacle").setup(function(pinnacle)
    ...
    input.keybind({ "Ctrl", "Alt", "Shift" }, keys.KEY_1, function()
        tag.switch_to("1")
    end)
    input.keybind({ "Ctrl", "Alt", "Shift" }, keys.KEY_2, function()
        tag.switch_to("2")
    end)
    input.keybind({ "Ctrl", "Alt", "Shift" }, keys.KEY_3, function()
        tag.switch_to("3")
    end)
    input.keybind({ "Ctrl", "Alt", "Shift" }, keys.KEY_4, function()
        tag.switch_to("4")
    end)
    input.keybind({ "Ctrl", "Alt", "Shift" }, keys.KEY_5, function()
        tag.switch_to("5")
    end)
end)
