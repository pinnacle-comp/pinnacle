                    ...
                    Some(dmabuf) => {
                        match state.dmabuf_imported(&data.dmabuf_global, dmabuf.clone()) {
                            Ok(()) => {
                                ...
                                let _ = data_init.init(id, dmabuf);
...
macro_rules! delegate_wl_drm {
    ($(@<$( $lt:tt $( : $clt:tt $(+ $dlt:tt )* )? ),+>)? $ty: ty) => {
        smithay::reexports::wayland_server::delegate_global_dispatch!($(@< $( $lt $( : $clt $(+ $dlt )* )? ),+ >)? $ty: [
            $crate::protocol::drm::wl_drm::WlDrm: $crate::protocol::drm::DrmGlobalData
        ] => $crate::protocol::drm::WlDrmState);
        smithay::reexports::wayland_server::delegate_dispatch!($(@< $( $lt $( : $clt $(+ $dlt )* )? ),+ >)? $ty: [
            $crate::protocol::drm::wl_drm::WlDrm: $crate::protocol::drm::DrmInstanceData
        ] => $crate::protocol::drm::WlDrmState);
    };
}
