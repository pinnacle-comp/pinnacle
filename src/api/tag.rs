...
pub fn switch_output(state: &mut State, tags_to_switch: Vec<Tag>, output_name: OutputName) {
    let Some(new_output) = output_name.output(&state.pinnacle) else {
        warn!(
            "Tried to switch tags to output {} but it doesn't exist",
            output_name.0
        );
        return;
    };
    ...
    for output in state.pinnacle.outputs.clone() {
        ...
        if output.name() == new_output.name() {
            output.with_state_mut(|state| {
                for tag in tags_to_switch.iter() {
